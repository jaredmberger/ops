...
if(backup.format!=='curator-ops-kv-recovery')fail('Unexpected backup format.');
...
console.log('Curator Ops recovery backup is valid.');
console.log(`Exported: ${backup.exportedAt||'unknown'}`);
console.log(`KV keys: ${backup.data.entries.length}`);
console.log(`SHA-256: ${actual}`);

function fail(message){console.error(`Invalid Curator Ops recovery backup: ${message}`);process.exit(1);}
