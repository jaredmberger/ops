...
const RUNTIMES = [
  ...
  { id:'content-opportunity', name:'Content Opportunity', url:'https://content-opportunity.oceanliners.net/api/runtime', repository:'jaredmberger/content-opportunity' }
];
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/runtime') {
      ...
      return json({ok:true,service:'Curator Ops',version:'1.1.3',repository:'jaredmberger/ops',runtime:'cloudflare-workers',cloudflareVersion:{id:meta.id||null,tag:meta.tag||null,timestamp:meta.timestamp||null},accessServiceAuthConfigured:Boolean(env.CF_ACCESS_CLIENT_ID&&env.CF_ACCESS_CLIENT_SECRET),observedAt:new Date().toISOString()});
    }
