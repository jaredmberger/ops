...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/runtime') {
      ...
      return json({ok:true,service:'Curator Ops',version:'1.1.5',repository:'jaredmberger/ops',runtime:'cloudflare-workers',cloudflareVersion:{id:meta.id||null,tag:meta.tag||null,timestamp:meta.timestamp||null},accessServiceAuthConfigured:Boolean(env.CF_ACCESS_CLIENT_ID&&env.CF_ACCESS_CLIENT_SECRET),observedAt:new Date().toISOString()});
    }
...
async function probeRuntime(service,env){const started=Date.now(),controller=new AbortController(),timer=setTimeout(()=>controller.abort(),REQUEST_TIMEOUT_MS);try{const target=new URL(service.url);target.searchParams.set('opsRuntime',Date.now().toString());const response=await fetch(target.href,{method:'GET',redirect:'manual',cache:'no-store',headers:{accept:'application/json','user-agent':'CuratorOps-Runtime/1.5 (+https://ops.oceanlinercurator.com)',...accessHeaders(env,target)},signal:controller.signal});const contentType=response.headers.get('content-type')||'';const location=response.headers.get('location')||null;if(response.status>=300&&response.status<400){throw new Error(`HTTP ${response.status} redirect${location?` → ${location}`:''}`)}if(!response.ok)throw new Error(`HTTP ${response.status}${contentType?` · ${contentType}`:''}`);if(!contentType.toLowerCase().includes('application/json')){const body=(await response.text()).slice(0,180).replace(/\s+/g,' ');throw new Error(`Expected JSON, got ${contentType||'unknown content type'}${body?` · ${body}`:''}`)}const p=await response.json();return{id:service.id,name:service.name,repository:p.repository||service.repository,ok:Boolean(p.ok),reportedService:p.service||null,version:p.version||null,runtime:p.runtime||null,cloudflareVersion:p.cloudflareVersion||null,build:p.build||null,observedAt:p.observedAt||null,checkedAt:new Date().toISOString(),durationMs:Date.now()-started}}catch(error){return{id:service.id,name:service.name,repository:service.repository,ok:false,error:error?.name==='AbortError'?'timeout':(error?.message||String(error)),checkedAt:new Date().toISOString(),durationMs:Date.now()-started}}finally{clearTimeout(timer)}}
