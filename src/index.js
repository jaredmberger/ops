...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname === '/') {
      ...
      return html(renderHome(snapshot));
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/check-now') {
      const result = await runChecks(env, 'manual');
      return json({ ok:true, snapshot:result });
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/heartbeat') {
      ...
      const record = await recordHeartbeat(env, body);
      return json({ ok:true, heartbeat:record }, 201);
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/deployment') {
      ...
      if (!auth.ok) return json({ ok:false, error:auth.error }, auth.status);
      const body = await readJson(request);
      const record = await recordDeployment(env, body);
      return json({ ok:true, deployment:record }, 201);
    }
  ...
  async scheduled(controller, env, ctx) {
    ctx.waitUntil(runChecks(env, `cron:${controller?.cron || 'unknown'}`).catch(error => console.error('Ops scheduled check failed', error)));
  }
};
...
async function runChecks(env, source) {
  ...
  const historyKey = `${HISTORY_PREFIX}${String(9999999999999 - ts).padStart(13,'0')}:${crypto.randomUUID()}`;
  await env[KV].put(historyKey, JSON.stringify(snapshot), { expirationTtl:60*60*24*30 });
  return snapshot;
}
...
async function readSnapshot(env) {
  requireKv(env);
  return await env[KV].get(SNAPSHOT_KEY, 'json') || {
    generatedAt:null,
    source:null,
    summary:{ total:SERVICES.length, healthy:0, degraded:0, status:'unknown' },
    services:SERVICES.map(s=>({ id:s.id, name:s.name, kind:s.kind, url:s.url, ok:null, status:null }))
  };
}
...
async function recordHeartbeat(env, body) {
  ...
  await env[KV].put(`${HEARTBEAT_PREFIX}${slug(service)}`, JSON.stringify(record), { expirationTtl:60*60*24*30 });
  return record;
}

async function recordDeployment(env, body) {
  requireKv(env);
  const service = clean(body?.service, 100);
  if (!service) throw new Error('service is required');
  const now = new Date().toISOString();
  const record = {
    ...
    commit:clean(body?.commit, 120) || null,
    version:clean(body?.version, 120) || null,
    environment:clean(body?.environment || 'production', 80),
    provider:clean(body?.provider || 'cloudflare', 80),
    source:clean(body?.source || 'reported', 80),
    deployedAt:clean(body?.deployedAt, 80) || now,
    recordedAt:now
  };
  const ts = Date.parse(record.deployedAt) || Date.now();
  const key = `${DEPLOYMENT_PREFIX}${slug(service)}:${String(9999999999999-ts).padStart(13,'0')}:${record.id}`;
  await env[KV].put(key, JSON.stringify(record), { expirationTtl:60*60*24*365 });
  await env[KV].put(`${DEPLOYMENT_PREFIX}latest:${slug(service)}`, JSON.stringify(record), { expirationTtl:60*60*24*365 });
  return record;
}

function authorizeWrite(request, env) {
  if (!env.OPS_WRITE_KEY) return { ok:false, status:503, error:'OPS_WRITE_KEY is not configured.' };
  const supplied = request.headers.get('x-curator-ops-key') || '';
  if (!supplied || supplied !== env.OPS_WRITE_KEY) return { ok:false, status:401, error:'Unauthorized.' };
  return { ok:true };
}
...
function renderHome(snapshot){
  const rows = (snapshot.services || []).map(s => `<tr><td><span class="dot ${s.ok===true?'ok':s.ok===false?'bad':'unknown'}"></span>${escapeHtml(s.name)}</td><td>${s.status ?? '—'}</td><td>${s.durationMs != null ? `${s.durationMs} ms` : '—'}</td><td>${s.checkedAt ? new Date(s.checkedAt).toLocaleString() : 'Not checked yet'}</td></tr>`).join('');
  const summary = snapshot.summary || {};
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Curator Ops</title><style>
  :root{color-scheme:dark;--bg:#0a1110;--panel:#101918;--brass:#bfa46a;--text:#f3eee3;--muted:#9aa6a0;--line:#263330}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 50% -20%,#17221f 0,#0a1110 52%);color:var(--text);font-family:Georgia,'Times New Roman',serif;min-height:100vh}.wrap{max-width:1050px;margin:0 auto;padding:58px 20px}.eyebrow{font:600 12px system-ui,sans-serif;letter-spacing:.18em;text-transform:uppercase;color:var(--brass)}h1{font-size:clamp(38px,7vw,64px);font-weight:400;margin:.18em 0}.lede{max-width:760px;color:#d3d6d1;font-size:18px;line-height:1.6}.cards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin:30px 0}.card,.table{border:1px solid var(--line);background:rgba(16,25,24,.84);border-radius:16px}.card{padding:20px}.label{font:600 11px system-ui,sans-serif;letter-spacing:.12em;text-transform:uppercase;color:var(--muted)}.value{font-size:30px;margin-top:8px}.table{overflow:hidden}table{width:100%;border-collapse:collapse;font:14px system-ui,sans-serif}th,td{text-align:left;padding:14px 16px;border-bottom:1px solid var(--line)}th{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.1em}.dot{display:inline-block;width:9px;height:9px;border-radius:50%;margin-right:9px}.ok{background:#58c77a}.bad{background:#d86666}.unknown{background:#8b9490}footer{margin-top:28px;color:#6f7974;font:12px system-ui,sans-serif}@media(max-width:700px){.cards{grid-template-columns:1fr}.table{overflow-x:auto}table{min-width:720px}}
