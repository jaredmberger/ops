...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/status') {
      ...
      return json({ ok:true, service:'Curator Ops', version:'1.0.2', generatedAt:new Date().toISOString(), displayTimeZone:DISPLAY_TIME_ZONE, accessServiceAuthConfigured:Boolean(env.CF_ACCESS_CLIENT_ID && env.CF_ACCESS_CLIENT_SECRET), snapshot });
    }
...
async function probe(service, env) {
  ...
  try {
    ...
    const response=await fetch(target.href,{method:'GET',redirect:'follow',cache:'no-store',headers:{'user-agent':'CuratorOps/1.0 (+https://ops.oceanlinercurator.com)',accept:'text/html,application/json,*/*;q=0.8',...accessHeaders(env,target)},signal:controller.signal,cf:{cacheTtl:0,cacheEverything:false}});
...
async function readSnapshot(env){requireKv(env);return await env[KV].get(SNAPSHOT_KEY,'json')||{generatedAt:null,source:null,displayTimeZone:DISPLAY_TIME_ZONE,summary:{total:SERVICES.length,healthy:0,degraded:0,status:'unknown'},services:SERVICES.map(s=>({id:s.id,name:s.name,kind:s.kind,url:s.url,ok:null,status:null}))}}
...
function renderHome(snapshot){const rows=(snapshot.services||[]).map(s=>`<tr><td><span class="dot ${s.ok===true?'ok':s.ok===false?'bad':'unknown'}"></span>${escapeHtml(s.name)}</td><td>${s.status??'—'}</td><td>${s.durationMs!=null?`${s.durationMs} ms`:'—'}</td><td>${s.checkedAt?formatCentral(s.checkedAt):'Not checked yet'}</td></tr>`).join(''),summary=snapshot.summary||{};return`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Curator Ops</title><style>:root{color-scheme:dark;--bg:#0a1110;--panel:#101918;--brass:#bfa46a;--text:#f3eee3;--muted:#9aa6a0;--line:#263330}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 50% -20%,#17221f 0,#0a1110 52%);color:var(--text);font-family:Georgia,'Times New Roman',serif;min-height:100vh}.wrap{max-width:1050px;margin:0 auto;padding:58px 20px}.eyebrow{font:600 12px system-ui,sans-serif;letter-spacing:.18em;text-transform:uppercase;color:var(--brass)}h1{font-size:clamp(38px,7vw,64px);font-weight:400;margin:.18em 0}.lede{max-width:760px;color:#d3d6d1;font-size:18px;line-height:1.6}.cards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin:30px 0}.card,.table{border:1px solid var(--line);background:rgba(16,25,24,.84);border-radius:16px}.card{padding:20px}.label{font:600 11px system-ui,sans-serif;letter-spacing:.12em;text-transform:uppercase;color:var(--muted)}.value{font-size:30px;margin-top:8px}.table{overflow:hidden}table{width:100%;border-collapse:collapse;font:14px system-ui,sans-serif}th,td{text-align:left;padding:14px 16px;border-bottom:1px solid var(--line)}th{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.1em}.dot{display:inline-block;width:9px;height:9px;border-radius:50%;margin-right:9px}.ok{background:#58c77a}.bad{background:#d86666}.unknown{background:#8b9490}.tz{color:var(--muted);font:12px system-ui,sans-serif;margin-top:8px}footer{margin-top:28px;color:#6f7974;font:12px system-ui,sans-serif}@media(max-width:700px){.cards{grid-template-columns:1fr}.table{overflow-x:auto}table{min-width:720px}}</style></head><body><main class="wrap"><div class="eyebrow">CuratorOS · Operational Control Plane</div><h1>Curator Ops</h1><p class="lede">Tracks the machinery behind CuratorOS: service reachability, runtime freshness, deployment reports, and operational drift. It does not judge site content.</p><section class="cards"><div class="card"><div class="label">Overall</div><div class="value">${escapeHtml(summary.status||'unknown')}</div></div><div class="card"><div class="label">Healthy</div><div class="value">${summary.healthy??0}/${summary.total??SERVICES.length}</div></div><div class="card"><div class="label">Last snapshot</div><div class="value" style="font-size:18px">${snapshot.generatedAt?formatCentral(snapshot.generatedAt):'Waiting for first check'}</div><div class="tz">US Central time</div></div></section><section class="table"><table><thead><tr><th>Service</th><th>HTTP</th><th>Latency</th><th>Checked · Central</th></tr></thead><tbody>${rows}</tbody></table></section><footer>Ocean Liner Curator · CuratorOS Operations Layer</footer></main></body></html>`}
