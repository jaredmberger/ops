...
`;
  ...
  out=out.replace(
    '<div class="links"><a href="/deployments">Deployment Drift →</a><a href="/scheduled">Scheduled Work →</a><a href="/api/status">Status JSON →</a></div>',
    '<div class="links"><a href="/deployments">Deployment Drift →</a><a href="/scheduled">Scheduled Work →</a><a href="/journey">Public Site Journey →</a><a href="/self-test">Self-Test →</a><a href="/history">Operational History →</a><a href="/api/status">Status JSON →</a></div>'
  );
  if(out.includes('<section class="table">')){
    out=out.replace('<section class="table">',section+'<section class="table">');
  }else if(out.includes('</main>')){
    out=out.replace('</main>',section+'</main>');
  }
  return out;
}
